...
function Sport() {
  ...
  useEffect(() => {
    const API_KEY = 'e5c4260c4a13e95e07e087750628b077'; // Replace with your own API key
    const url = `https://api.mediastack.com/v1/news?access_key=${API_KEY}&countries=in&categories=sports`; // Fetch sports news from India

    const fetchNews = async () => {
      try {
        const response = await axios.get(url);
        setArticles(response.data.data); // Adjust to MediaStack's response structure
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold my-8 text-center text-gray-800">Sports News</h1>
